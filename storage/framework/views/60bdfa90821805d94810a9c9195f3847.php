...
<div class="space-y-8">
    ...
    <div class="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
        <div class="flex justify-between items-start">
            <div>
                <span class="bg-blue-100 text-blue-700 text-xs font-bold px-3 py-1 rounded-full uppercase tracking-widest"><?php echo e($deal->company->industry); ?></span>
                <h1 class="text-4xl font-black text-slate-900 mt-4"><?php echo e($deal->is_confidential && !$canViewPrivate ? 'Project ' . strtoupper(substr($deal->title, 0, 4)) : $deal->title); ?></h1>
                <p class="text-slate-500 mt-2 flex items-center"><i class="fa fa-location-dot mr-2"></i> <?php echo e($deal->location ?? 'Global / Remote'); ?></p>
            </div>
            <div class="text-right">
                ...
                <div class="flex items-center justify-end gap-4 mt-1">
                    <p class="text-4xl font-black text-blue-600">$<?php echo e(number_format($deal->asking_price)); ?></p>
                    <?php if(auth()->guard()->check()): ?>
                        <?php if(auth()->user()->role === 'buyer'): ?>
                        <form action="<?php echo e(route('deals.bookmark', $deal)); ?>" method="POST">
                            <?php echo csrf_field(); ?>
                            <button type="submit" class="p-2 rounded-full border border-slate-200 hover:bg-slate-50 <?php echo e(auth()->user()->bookmarkedDeals->contains($deal->id) ? 'text-blue-600' : 'text-slate-400'); ?>">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-8">
            <div class="bg-white p-8 rounded-2xl border border-slate-200">
                ...
                <p class="text-slate-600 leading-relaxed"><?php echo e($deal->teaser); ?></p>
            </div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div class="bg-white p-6 rounded-2xl border border-slate-200">
                    ...
                    <p class="text-xl font-black">$<?php echo e(number_format($deal->revenue_annual)); ?></p>
                </div>
                <div class="bg-white p-6 rounded-2xl border border-slate-200">
                    <p class="text-xs font-bold text-slate-400 uppercase">EBITDA</p>
                    <p class="text-xl font-black">$<?php echo e(number_format($deal->ebitda)); ?></p>
                </div>
                <div class="bg-white p-6 rounded-2xl border border-slate-200">
                    <p class="text-xs font-bold text-slate-400 uppercase">Net Profit</p>
                    <p class="text-xl font-black">$<?php echo e(number_format($deal->net_profit ?? 0)); ?></p>
                </div>
                <div class="bg-white p-6 rounded-2xl border border-slate-200">
                    <p class="text-xs font-bold text-slate-400 uppercase">Growth</p>
                    <p class="text-xl font-black text-emerald-600">+<?php echo e($deal->growth_percentage ?? 0); ?>%</p>
                </div>
            </div>
            ...
            <?php if($canViewPrivate): ?>
                <div class="bg-emerald-50 border border-emerald-200 p-8 rounded-2xl">
                    ...
                    <div class="space-y-6">
                        ...
                        <?php $__empty_1 = true; $__currentLoopData = $groupedDocuments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $folder => $documents): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <div>
                            <h3 class="text-sm font-bold text-emerald-800 uppercase tracking-widest mb-3 flex items-center"><i class="fa fa-folder text-emerald-500 mr-2"></i> <?php echo e($folder); ?></h3>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div class="bg-white p-4 rounded-xl border border-emerald-100 flex items-center justify-between">
                                    <div class="flex items-center">
                                        <?php if(str_contains($document->file_type, 'pdf')): ?>
                                            <i class="fa fa-file-pdf text-red-500 text-2xl mr-3"></i>
                                        <?php elseif(str_contains($document->file_type, 'word') || str_contains($document->file_type, 'doc')): ?>
                                            <i class="fa fa-file-word text-blue-500 text-2xl mr-3"></i>
                                        <?php elseif(str_contains($document->file_type, 'excel') || str_contains($document->file_type, 'xls')): ?>
                                            <i class="fa fa-file-excel text-emerald-500 text-2xl mr-3"></i>
                                        <?php else: ?>
                                            <i class="fa fa-file text-slate-500 text-2xl mr-3"></i>
                                        <?php endif; ?>
                                        <div>
                                            <p class="font-bold text-sm"><?php echo e($document->title); ?></p>
                                            <p class="text-[10px] text-slate-400"><?php echo e(number_format($document->file_size / 1024, 1)); ?> KB • <?php echo e(strtoupper($document->file_type)); ?></p>
                                        </div>
                                    </div>
                                    <a href="/storage/<?php echo e($document->file_path); ?>" target="_blank" class="text-slate-400 hover:text-emerald-600"><i class="fa fa-download"></i></a>
                                </div>
                    ...
                    <div class="mt-8">
                        ...
                        <form action="/deals/<?php echo e($deal->id); ?>/offers" method="POST" class="space-y-4">
                            <?php echo csrf_field(); ?>
            ...
            <?php else: ?>
                <div class="bg-slate-900 p-12 rounded-2xl text-center">
                    ...
                    <?php if(auth()->guard()->check()): ?>
                        <form action="/deals/<?php echo e($deal->id); ?>/nda" method="POST" class="mt-8">
                            <?php echo csrf_field(); ?>
        ...
        <div class="space-y-6">
            <div class="bg-white p-6 rounded-2xl border border-slate-200">
                ...
                <div class="space-y-4">
                    <?php $__empty_1 = true; $__currentLoopData = $deal->company->shareholders; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $shareholder): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <div class="flex justify-between items-center text-sm">
                        <span class="text-slate-500"><?php echo e($shareholder->name); ?> (<?php echo e($shareholder->type); ?>)</span>
                        <div class="flex items-center">
                            <div class="w-24 h-2 bg-slate-100 rounded-full mr-2 overflow-hidden">
                                <div class="bg-blue-600 h-full" style="width: <?php echo e($shareholder->ownership_percentage); ?>%"></div>
                            </div>
                            <span class="font-bold"><?php echo e($shareholder->ownership_percentage); ?>%</span>
                        </div>
                    </div>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH H:\laragon\www\m&a-marketplace-laravel-backend\resources\views/deals/show.blade.php ENDPATH**/ ?>