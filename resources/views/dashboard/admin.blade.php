...
<div class="space-y-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div class="bg-white p-6 rounded-2xl border border-slate-200">
            ...
            <p class="text-3xl font-black mt-2">{{ number_format($totalUsers) }}</p>
        ...
        <div class="bg-white p-6 rounded-2xl border border-slate-200">
            ...
            <p class="text-3xl font-black mt-2">{{ number_format($activeDeals) }}</p>
        ...
        <div class="bg-white p-6 rounded-2xl border border-slate-200">
            ...
            <p class="text-3xl font-black mt-2 text-orange-600">{{ number_format($pendingKyc) }}</p>
        ...
        <div class="bg-white p-6 rounded-2xl border border-slate-200">
            ...
            <p class="text-3xl font-black mt-2">{{ number_format($ndasSigned) }}</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div class="bg-white p-8 rounded-2xl border border-slate-200">
            ...
            <div class="space-y-4">
                @forelse($pendingDeals as $pendingDeal)
                <div class="p-4 bg-slate-50 rounded-xl border border-slate-100 flex items-center justify-between">
                    <div>
                        <p class="font-bold text-sm">{{ $pendingDeal->title }}</p>
                        <p class="text-xs text-slate-400">Seller: {{ $pendingDeal->company->seller->name ?? 'Unknown' }} • ${{ number_format($pendingDeal->asking_price) }}</p>
                    </div>
