    ...
    <div class="flex justify-between items-end">
        ...
        <form action="/" method="GET" class="flex flex-wrap items-center gap-3">
            ...
            <select name="deal_type" class="px-4 py-2 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none bg-white text-sm">
                ...
                <option value="sale_100" {{ request('deal_type') == 'sale_100' ? 'selected' : '' }}>100% Acquisition</option>
                <option value="partial_sale" {{ request('deal_type') == 'partial_sale' ? 'selected' : '' }}>Partial Sale</option>
                <option value="fundraising" {{ request('deal_type') == 'fundraising' ? 'selected' : '' }}>Fundraising</option>
            </select>

            <input type="text" name="location" value="{{ request('location') }}" placeholder="Location..." class="px-4 py-2 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none text-sm w-32">
            
            <div class="flex items-center gap-2">
                <input type="number" name="min_price" value="{{ request('min_price') }}" placeholder="Min $..." class="px-4 py-2 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none text-sm w-28">
                <span class="text-slate-400">-</span>
                <input type="number" name="max_price" value="{{ request('max_price') }}" placeholder="Max $..." class="px-4 py-2 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none text-sm w-28">
            </div>
            ...
            @if(request()->anyFilled(['search', 'industry', 'location', 'deal_type', 'min_price', 'max_price']))
